...
public class DarkBlueAlphaBeta extends StateMachineGamer {
	...
	public void stateMachineMetaGame(long timeout)
		...
		// TODO Auto-generated method stub
		//Use Reinforcement learning (combining heuristic functions)
	...
	public Move stateMachineSelectMove(long timeout)
		...
		if(machine.getRoles().size() == 1) {
			return bestMoveCompulsive(machine, role, state);
		}
		else {
	...
	public static Move bestMoveCompulsive(StateMachine machine, Role role, MachineState state)
		...
		for(int i = 0; i < states.size(); i++) {
			...
			int result = maxscoreCompulsive(machine, role, next_state);
			if(result > score) {
				score = result;
				move = moves.get(i);
			}
	...
	public static int maxscoreCompulsive(StateMachine machine, Role role, MachineState state)
			throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException {
		if(machine.isTerminal(state)) return machine.getGoal(state, role);
		List<MachineState> states = machine.getNextStates(state);
		int score = 0;
		for(int i = 0; i < states.size(); i++) {
			int result = maxscoreCompulsive(machine, role, states.get(i));
