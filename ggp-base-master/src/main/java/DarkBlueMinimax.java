...
public class DarkBlueMinimax extends StateMachineGamer {
	...
	public static Move bestMove(StateMachine machine, Role role, MachineState state)
		...
		for(int i = 0; i < moves.size(); i++) {
			...
			int result = minscore(machine, role, nextMove, state);
			if(result > score) {
				score = result;
				move = moves.get(i);
			}
	...
	public static int minscore(StateMachine machine, Role role, Move move, MachineState state)
...
//		List<Role> roles = machine.getRoles();
		...
		*/
		...
		for (int i = 0; i < moves.size(); i++)
		{
				...
				int result = maxscore(machine, role, newState);
				if (result < score){
					score = result;
				}
		}
	...
	public static int maxscore(StateMachine machine, Role role, MachineState state)
		...
		for(int i = 0; i < moves.size(); i++) {
			...
			int result = minscore(machine, role, moves.get(i), state);
